...
public class PlayerTest {
	...
	public void testDefaultConstructor() {
		...
		assertTrue("Player's rollable field should be initialised with an implementing instance of the Rollable " +
				"interface", p.getRollable() instanceof Rollable);
	}
	...
	public void testCustomConstructor1arg() {
		...
		assertSame("Player's name field should be initialised with and return the same object received by the " +
				"constructor", n, p.getName());
		assertTrue("Player's rollable field should be initialised with an implementing instance of the Rollable " +
				"interface", p.getRollable() instanceof Rollable);
	}
	...
	public void testCustomConstructor2argsWithDie() {
		...
		assertSame("Player's name field should be initialised with and return the same object received by the " +
				"constructor", n, p.getName());
		assertSame("Player's rollable field should be initialised with and return the same object received by the " +
				"constructor", r, p.getRollable());
	}
	...
	public void testRollDice() {
		...
		//attempts to roll the player's dice up to 1000 times, until the score of the Rollable reference received by
		...
		//if the score was not updated then the roll method cannot be working - you're not going to roll the same
		// score 1000 times!
		if (i == 1000) {
	...
	/* To ensure the score returned cannot be hardcoded, this test method firstly checks the initial score matches
    that of the
	...
	public void testGetDiceScore() {
		...
		int initialScore = r.getScore();
		assertEquals("The getDiceScore method is not working so rollDice cannot be tested properly", initialScore,
				p.getDiceScore());
	...
	public void testSetFullPlayerName() {
		...
		assertEquals("The Name object held by the Player should have its first name set to Joe", "Joe",
				p.getName().getFirstName());
		assertEquals("The Name object held by the Player should have its family name set to Bloggs", "Bloggs",
		...
		assertEquals("The Name object held by the Player should have its first name set to Tim", "Tim",
				p.getName().getFirstName());
		assertEquals("The Name object held by the Player should have its family name set to Jones", "Jones",
				p.getName().getFamilyName());
	}
